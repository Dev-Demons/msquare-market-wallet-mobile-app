...
class UserAPIServices {
  ...
  * */
  ...
  Future fetchCurrentUser() async {
    ...
    try {
      ...
      // if status code of HttpResponse is 404,500,503
        ...
        throw "${_response.statusCode} User not found";
      } else {
  ...
  Future updatePassword(
    ...
    _token = await locateUser.getUserToken();
    _body = jsonEncode({
      "user": locateUser.firebaseUser!.email,
    ...
    _url = Uri.parse('${EndPoints.updatePassword}');
    _response = await http.post(_url!, headers: _localHeaders, body: _body);
  ...
  Future announcement() async {
    ...
    _url = Uri.parse('${EndPoints.announcement}');
    _response = await http.get(_url!, headers: _localHeaders);
    if (_response.statusCode == 200) {
      return {"error": false, "data": jsonDecode(_response.body)};
    } else {
      return {"error": true, "data": jsonDecode(_response.body)};
    }
  }
}
