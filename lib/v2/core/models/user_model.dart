...
class UserModel {
  ...
  updateTokenInfo(Map userTokens, Map tokenPrices) async {
    ...
    wallet!.tokens!.forEach((element) {
      totalBalanceInKRW =
          totalBalanceInKRW + (element.balance! * element.price!);
      totalBalanceTrend =
...
class Wallet {
  ...
  List sequence = ['MSQ', 'MSQP', 'MSQX', 'MSQXP', 'P2UP', 'MATIC'];
  ...
  Wallet.fromJson(Map json) {
    ...
    offChainAddress = json['msqaddress'];
  }
  ...
  sortInSequence() async {
    Token? swap;
    for (int i = 0; i < sequence.length && i < tokens!.length; ++i) {
